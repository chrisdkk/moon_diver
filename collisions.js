...
function collisionTop(y, xMin, xMax, boxHeight) {
  if (player.x - player.radius < xMax && player.x + player.radius > xMin) {
    if (
      player.y + player.dY > y - player.radius &&
      player.y + player.dY < y + boxHeight - player.radius
    ) {
      if (player.dY > 1) {
      ...
      if (player.y + player.dY > y - player.radius) {
        player.colDY = player.y + player.dY - y - player.radius;
      }
    }
  }
}
...
function collisionLeft(x, yMin, yMax, boxWidth) {
  if (player.y - player.radius < yMax && player.y + player.radius > yMin) {
    if (
      player.x + player.dX > x - player.radius &&
      player.x + player.dX < x - player.radius + boxWidth
    ) {
      player.dX = -player.dX / 3;
      if (player.x + player.dX > x - player.radius) {
        player.colDX = player.x + player.dX - x - player.radius;
      }
    }
  }
}
